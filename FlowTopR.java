...
public class FlowTopR extends TypedAtomicActor {
	public int enablesend = 0;
	...
	//------------------------------------
	public FlowTopR(CompositeEntity container, String name) throws               
...
//---------check for injected task--------------------------------------//		
		if (_injectport.hasToken(0) && enablesend == 1) {				
...
//----------------check south port for data-----------------------------------------//		
	...
	public void outselect() throws IllegalActionException {
								...
								// { {bestW,bestS,bestI}, {bestW,bestS,bestI}, {bestW,bestS,bestI} } 
		...
		for (int i = P_levels-1; i >= 0; i--) {
			...
			//find highest priority in west buffer
			if (currentheaderW[i][1] > 0) {
	...
	public void pruneDependencies() {
		...
		removeDependency(inputDataW, outputDataS);
		removeDependency(inputDataW, outputCredS);
		removeDependency(inputDataW, inputCredS);
		removeDependency(inputCredW, outputDataS);
		removeDependency(inputCredW, outputCredS);
	}
}
